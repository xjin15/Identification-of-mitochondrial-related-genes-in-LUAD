...
mads <- mads[mads>0]
names(mads)
mads <- mads[intersect(names(mads),siggene)]
names(mads)

#d=d[rev(order(mads)),] # 取差异最大的前5000个基因
rt1_TP <- rt1_TP[rev(order(mads))[1:500],] %>% as.matrix()
#sweep函数减去中位数进行标准化
...
data1 <- cln
p_load(survival,survminer,ConsensusClusterPlus)
...
##### 使用全部1626个基因做聚类，选择KM方法#####


rt2_TP <- rt_TP
dd2 <- rt2_TP %>% as.matrix()

results2 = ConsensusClusterPlus(dd2,maxK=6,
                               reps=1000,pItem=0.8, # 抽样比例为0.8
                               pFeature=1,
                               title="output/MITOtest/1626/",
                               clusterAlg="km", # 聚类的方法改成km试试
                               distance="pearson",
                               plot="png")

groupmito <- results2[[2]][["consensusClass"]]
cln <- read.csv(file = "data/LUAD_survival.csv")
cln <- cln[cln$sample %in% names(groupmito),]
cln$group <- groupmito[base::match(cln$sample, names(groupmito),nomatch = 0)]
cln$group %>% table()
...
##### 使用siggene 1269个基因做聚类，选择KM方法，得到聚类分组######

library(ConsensusClusterPlus)
intersect(rownames(rt_TP), siggene) %>% length()
rt2_TP <- rt_TP[siggene,]
dd3 <- rt2_TP %>% as.matrix()
maxK = 6
results3 = ConsensusClusterPlus(dd3,maxK=maxK,
                                reps=1000,pItem=0.8, # 抽样比例为0.8
                                pFeature=1,
                                title="output/MITOtest/1269/",
                                clusterAlg="km", # 聚类的方法改成km试试
                                distance="euclidean ", # km方法只支持欧式距离euclidean
                                plot="png")

###### 用PAC法确定k值。但记住PAC法可能高估K值，因此不要完全信任
Kvec = 2:maxK
x1 = 0.1; x2 = 0.9 # threshold defining the intermediate sub-interval
PAC = rep(NA,length(Kvec)) 
names(PAC) = paste("K=",Kvec,sep="") # from 2 to maxK
for(i in Kvec){
  M = results3[[i]]$consensusMatrix
...
groupmito <- results3[[2]][["consensusClass"]]
cln <- read.csv(file = "data/LUAD_survival.csv")
cln <- cln[cln$sample %in% names(groupmito),]
cln$group <- groupmito[base::match(cln$sample, names(groupmito),nomatch = 0)]
cln$group %>% table()
...
# data1 <- cln %>% filter(OS.time<=62)
data1 <- cln %>% filter(OS.time<84)# 只选择生存时间小于100个月的
p_load(survival,survminer)
fit<-survfit(Surv(OS.time,OS)~group, data = data1)
...
pp <- ggsurvplot(fit, data=data1, linetype = 1, palette = c("jco"),size=1,
                 surv.scale = c("percent"),pval = TRUE,
                 legend.title = '', legend.labs=c("1","2"),
                 break.time.by =12,
                 # xlim = c(0,60),
                 risk.table = TRUE,
                 risk.table.title = "Patients at risk",
...
print(pp)
library(export)
graph2ppt(file="output/plots/OS.pptx",append = T)
library(eoffice)
topptx(filename = "output/plots/siggeneOS.pptx", append = T) 
todocx(pp,filename = "output/plots/siggeneOS", ) 
toffice(pp,format = "pptx",filename = "output/plots/siggeneOS", ) 


# 保存分组id ------------------------------------------------------------------
library(tidyverse)
allid <- cln |> arrange(group) |> select(sample) |> pull()
sample_group <- cln |> arrange(group) |> select(sample,group)
save(allid,sample_group,file = "outdata/step4_group_data.rds")
