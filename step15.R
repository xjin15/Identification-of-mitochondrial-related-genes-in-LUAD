...
dt <- phe_f |> filter(sample %in% sample_group$sample) |> 
  dplyr::select( sample, starts_with("age"), gender,race, starts_with("smoke"),
          starts_with("ajcc"), starts_with("stage"),
          radiotherapy, resection_site) |> 
  mutate_(group = sample_group$group[match(sample,sample_group$sample)])
...
dt_factor <- dt[,-1] |> 
  transmute(
    ...
    Age = as.numeric(age),
    Age_median = as.factor(age_median),
    Age_group  = as.factor(age_group2),
...
# OS和PFS ------------------------------------------------------------------
p_load(survival, survminer, export)
